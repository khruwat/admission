// v1_22_4: เพิ่ม LINE_GROUP_URL, LINE_QR_URL ใน SETTINGS_DEFAULTS
/* =============================================================
   ระบบรับสมัครนักเรียน โรงเรียนหนองนาคำวิทยาคม
   Student Enrollment System  v1.22.10
   ─────────────────────────────────────────────────────────────
...
function submitApplication(data) {
  ...
  const cfg      = getSettings();
  // อ่าน TEST_MODE โดยตรงจาก Sheet เพื่อให้แน่ใจว่าได้ค่าล่าสุด (ไม่ติดค้างใน cache)
  const testMode = _isTestModeActive();

  // ตรวจ checksum บัตรประชาชน — ข้ามเมื่อ TEST_MODE=true (ต้องกรอกครบ 13 หลักเสมอ)
  ...
  // ย้ายเข้า หรือ TEST_MODE — ไม่ตรวจวันเปิด/ปิด
...
function _buildEnrollmentPdfHtml(a, cfg) {
  ...
  // ── แปลง Drive viewUrl → thumbUrl สำหรับ <img> ──
  ...
  var logoUrl  = _fixDriveUrl(cfg.LOGO_URL  || '');
  var photoUrl = _fixDriveUrl(a.PHOTO_URL   || '');

  // ── helpers ──
  // _v: แปลงค่าเป็น safe string (ไม่ผ่าน _pdfSafe ซ้ำเพราะค่าอาจเป็น HTML ที่ต้องการ)
  ...
  // _vRaw: ใช้สำหรับ field ที่ต้องการ HTML (เช่น โรงเรียนเดิม+ที่อยู่)
  var _vRaw = function(v){ return (v !== null && v !== undefined && String(v).trim() !== '') ? String(v).trim() : '–'; };
  // _phone: บังคับแปลงเบอร์โทรเป็น string เสมอ (ป้องกัน 0 หายตอน Sheet เก็บเป็น number)
  var _phone = function(v){
    var s = String(v === null || v === undefined ? '' : v).trim();
    if (!s || s === '0') return '–';
    // ถ้า Sheet เก็บเป็น number เช่น 989009887 → เติม 0 นำหน้า
    if (/^\d{9}$/.test(s)) s = '0' + s;
    return _pdfSafe(s);
  };
  ...
  if (guardianType === 'บิดา') {
    gName = fatherName; gRel = 'บิดา'; gPhone = _phone(a.FATHER_PHONE);
  } else if (guardianType === 'มารดา') {
    gName = motherName; gRel = 'มารดา'; gPhone = _phone(a.MOTHER_PHONE);
  } else {
    gName = _v(a.GUARDIAN_NAME); gRel = _v(a.GUARDIAN_RELATION); gPhone = _phone(a.GUARDIAN_PHONE);
  }
  var healthBits = [
  ...
  // ── โรงเรียนเดิม: ต้องแสดงที่อยู่เป็น plain text ไม่ใช่ HTML ──
  var _oldSchoolVal = function(){
    var s = String(a.OLD_SCHOOL || '').trim();
    if (!s) return '–';
    var parts = [
      String(a.OLD_SCHOOL_SUBDISTRICT || '').trim(),
      String(a.OLD_SCHOOL_DISTRICT    || '').trim(),
      String(a.OLD_SCHOOL_PROVINCE    || '').trim()
    ].filter(Boolean);
    var addr = parts.join(' ');
    // ใช้ _pdfSafe ทั้ง school name และ addr (ป้องกัน HTML tag โผล่)
    return addr ? (_pdfSafe(s) + ' (' + _pdfSafe(addr) + ')') : _pdfSafe(s);
  };
  ...
  var rowHtml = function(l, v){
    // ใช้สำหรับ value ที่ผ่าน _pdfSafe แล้ว แต่ต้องการ control layout เอง
    return '<div class="frow"><div class="flabel">' + _pdfSafe(l) + '</div><div class="fvalue">' + (v || '–') + '</div></div>';
  };
  ...
  var row2p = function(l1, v1, l2, v2){
    // row2 แต่ v1, v2 ผ่าน _phone แล้ว
    return '<div class="frow two"><div class="pair"><div class="flabel">' + _pdfSafe(l1) + '</div><div class="fvalue">' + v1 + '</div></div><div class="pair"><div class="flabel">' + _pdfSafe(l2) + '</div><div class="fvalue">' + v2 + '</div></div></div>';
  };

  var logoHtml  = logoUrl  ? '<img src="' + logoUrl  + '" style="width:100%;height:100%;object-fit:contain;display:block" crossorigin="anonymous">'
                           : '<div class="logo-fallback">🏫</div>';
  var photoHtml = photoUrl ? '<img src="' + photoUrl + '" style="width:100%;height:100%;object-fit:cover;display:block" crossorigin="anonymous">'
                           : '<div class="photo-placeholder"><div>ติดรูปถ่าย</div><div>นักเรียน 1 นิ้ว</div></div>';

  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8">' +
'<link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@400;600;700&display=swap" rel="stylesheet">' +
'<style>' +
'*{box-sizing:border-box;margin:0;padding:0} @page{size:A4 portrait;margin:8mm} ' +
'html,body{background:#fff;color:#111;font-family:"Sarabun",sans-serif;font-size:10.6pt;line-height:1.4;-webkit-print-color-adjust:exact;print-color-adjust:exact} ' +
'.sheet{width:100%;min-height:auto;background:#fff;padding:0;position:relative} ' +
'.sheet::after{content:"' + _v(a.APP_ID) + '";position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:54pt;font-weight:700;color:rgba(26,58,107,.035);transform:rotate(-30deg);pointer-events:none;z-index:0} ' +
...
'.grid{display:grid;grid-template-columns:1.08fr .92fr;gap:3mm;position:relative;z-index:1}.section{border:1pt solid #bfcada;margin-bottom:3mm}.section-title{background:#163a6b;color:#fff;padding:1.6mm 2.4mm;font-size:8.8pt;font-weight:700;letter-spacing:.2px}.section-body{padding:1.6mm 2.2mm 1.8mm}.frow{display:grid;grid-template-columns:31mm 1fr;border-bottom:.6pt solid #d6deea;min-height:7mm}.frow:last-child{border-bottom:none}.frow.two{grid-template-columns:1fr 1fr;gap:2mm}.pair{display:grid;grid-template-columns:22mm 1fr}.flabel{background:#f3f6fb;padding:1.4mm 1.8mm;font-size:8.2pt;font-weight:700;color:#314055;display:flex;align-items:center}.fvalue{padding:1.4mm 1.8mm;display:flex;align-items:center;flex-wrap:wrap;min-height:7mm;word-break:break-word}.mono{font-family:"Courier New",monospace;letter-spacing:.5px} ' +
'.sub-title{font-size:8.8pt;font-weight:700;color:#163a6b;margin:1mm 0}.sub-gap{margin-top:2mm}.band{border:1pt solid #bfcada;padding:1.8mm 2.4mm;margin-bottom:3mm;position:relative;z-index:1}.band-title{font-size:8.8pt;font-weight:700;color:#163a6b;margin-bottom:1mm}.band-text{font-size:8.7pt;line-height:1.5}.check-grid{display:grid;grid-template-columns:1fr 1fr;gap:1.4mm 8mm;margin-top:1mm}.check-item{font-size:8.2pt;display:flex;align-items:flex-start;gap:6px}.checkbox{width:10px;height:10px;border:1px solid #555;display:inline-block;flex:0 0 auto;margin-top:2px} ' +
...
row2p('โทรศัพท์นักเรียน', _phone(a.PHONE), 'โรงเรียนเดิม', _oldSchoolVal()) +
...
'<div class="sub-title">บิดา</div>' + row('ชื่อ-สกุล', fatherName) + row2p('สถานะ', _v(a.FATHER_STATUS), 'โทรศัพท์', _phone(a.FATHER_PHONE)) + row2('อาชีพ', a.FATHER_OCCUPATION, 'รายได้', inc(a.FATHER_INCOME)) +
'<div class="sub-title sub-gap">มารดา</div>' + row('ชื่อ-สกุล', motherName) + row2p('สถานะ', _v(a.MOTHER_STATUS), 'โทรศัพท์', _phone(a.MOTHER_PHONE)) + row2('อาชีพ', a.MOTHER_OCCUPATION, 'รายได้', inc(a.MOTHER_INCOME)) +
'<div class="sub-title sub-gap">ผู้ปกครอง</div>' + row('ชื่อ-สกุล', gName) + row2p('ความเกี่ยวข้อง', gRel, 'โทรศัพท์', gPhone) +
