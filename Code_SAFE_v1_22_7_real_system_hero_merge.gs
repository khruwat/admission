...
function uploadHeroBg(token, base64Data) {
  _requireAdmin(token);
  if (!base64Data || typeof base64Data !== 'string')
    return { ok:false, msg:'ไม่ได้รับข้อมูลรูปภาพ' };
  ...
  try {
    ...
    const bytes = Utilities.base64Decode(raw);
    const blob = Utilities.newBlob(bytes, type, 'hero_bg_school.' + ext);
    ...
    ['png','jpg','jpeg','gif','webp'].forEach(function(e) {
      const iter = folder.getFilesByName('hero_bg_school.' + e);
      while (iter.hasNext()) iter.next().setTrashed(true);
    });
    ...
    updateSettings(token, { HERO_BG_URL: url });
    _logWithRole('ADMIN', 'ADMIN', 'UPLOAD_HERO_BG', 'fileId:' + fileId);
    return { ok:true, url:url, thumbUrl:urls.thumbUrl, downloadUrl:urls.downloadUrl, fileId:fileId };
  } catch(e) {
    Logger.log('uploadHeroBg error: ' + e);
    return { ok:false, msg:'Upload ไม่สำเร็จ: ' + String(e) };
  }
}
