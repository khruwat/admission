/* =============================================================
   ...
   Student Enrollment System  v1.9.0
   ─────────────────────────────────────────────────────────────
...
const SETTINGS_DEFAULTS = [
  ...
  ['CHANGELOG_TEXT',     'v1.9.0 – 2569-03-17 – ยืนยันตัวตนด้วยวันเกิดก่อนแก้ไขใบสมัคร\nv1.8.0 – 2569-03-17 – อัปเกรดระบบที่อยู่ jquery.Thailand.js ครอบทุกจังหวัด\nv1.7.0 – 2569-03-16 – Supabase dual-write, real-time sync'],
  ['ANNOUNCEMENT',       ''],
...
function verifyOwnerAndGetApp(idCard, birthdate) {
  ...
  const safe = {};
  [
    'APP_ID','LEVEL','APP_TYPE','PREFIX','FNAME','LNAME','ID_CARD','BIRTHDATE',
...
function updateApplicationByStudent(idCard, birthdate, appId, updates) {
  ...
  // ห้ามแก้ไขถ้าสถานะ ผ่าน หรือ มีสิทธิ์สอบ
  if (['ผ่าน','มีสิทธิ์สอบ'].includes(verify.app.STATUS))
    return { ok:false, msg:'ไม่สามารถแก้ไขได้ เนื่องจากมีสิทธิ์สอบแล้ว กรุณาติดต่อเจ้าหน้าที่' };

  // whitelist — แก้ไขได้เฉพาะฟิลด์เหล่านี้เท่านั้น
  ...
  ALLOWED_FIELDS.forEach(function(k) {
    if (updates[k] !== undefined && updates[k] !== null) {
      // income เก็บเป็น number; ที่เหลือเป็น string
      safeUpdates[k] = (k.endsWith('_INCOME') && updates[k] !== '')
        ? _toNum(updates[k])
        : String(updates[k]);
    }
  });
