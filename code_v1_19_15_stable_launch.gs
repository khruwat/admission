/* =============================================================
   ...
   Student Enrollment System  v1.19.0
   ─────────────────────────────────────────────────────────────
...
const SETTINGS_DEFAULTS = [
  ...
  ['DEVELOPER_POSITION', ''],
  ['DEVELOPER_PHONE',    ''],
  ['SYSTEM_VERSION',     'v1.19.2'],
  ['CHANGELOG_TEXT',     'v1.19.2 – 2569-03-17 – Fix: uploadLogo\nv1.19.1 – 2569-03-17 – Bugfix addAdminUser + exportScoreSheetUrl\nv1.19.0 – 2569-03-17 – ระบบคะแนนสอบ + Export Google Sheet'],
  ['ANNOUNCEMENT',       ''],
  ['ANNOUNCE_IMAGE',     ''],
  ['ANNOUNCE_POPUP',     'true'],
...
const _HOME_STATS_CACHE = 'home_stats_v1';
const _PUB_STATS_CACHE = 'pub_stats_v1';

function getHomeStats() {
  const cached = _CACHE.get(_HOME_STATS_CACHE);
  if (cached) {
  ...
  var midPlans  = String(cfg.STUDY_PLANS_MID || 'แผนการเรียนทั่วไป').split('\n').map(function(s){ return s.trim(); }).filter(Boolean);
  var highPlans = String(cfg.STUDY_PLANS_HIGH || 'วิทยาศาสตร์-คณิตศาสตร์\nศิลปะ-ภาษา\nศิลปะ-การงานอาชีพ').split('\n').map(function(s){ return s.trim(); }).filter(Boolean);

  const result = {
    m1:           zeroLevelStat(cfg.M1_QUOTA),
    m4:           zeroLevelStat(cfg.M4_QUOTA),
    total:        0,
    ...
    studyPlansMid:  midPlans,
    studyPlansHigh: highPlans,
    studyPlansByLevel: {
      'ม.1': _plans('STUDY_PLANS_M1', midPlans),
      'ม.2': _plans('STUDY_PLANS_M2', midPlans),
      'ม.3': _plans('STUDY_PLANS_M3', midPlans),
      'ม.4': _plans('STUDY_PLANS_M4', highPlans),
      'ม.5': _plans('STUDY_PLANS_M5', highPlans),
      'ม.6': _plans('STUDY_PLANS_M6', highPlans),
    },
    serviceSchools: String(cfg.SERVICE_AREA_SCHOOLS || '').split('\n').map(function(s){ return s.trim(); }).filter(Boolean),
    statsMode: 'settings-only'
  };

  _CACHE.put(_HOME_STATS_CACHE, JSON.stringify(result), 300);
  return result;
}
...
function getPublicStats() {
  ...
  function levelStat(arr, quota) {
    return {
      ...
      รายงานตัวแล้ว: count(arr, a => a.STATUS === 'รายงานตัวแล้ว'),
    };
  }
  ...
  const topSchools = Object.entries(schoolCount).sort((a,b)=>b[1]-a[1]).slice(0,10).map(([name,count])=>({name,count}));
  ...
  const days30 = {};
  Array.from(allDateSet).sort().forEach(function(d){
    days30[d] = { total:0, m1:{ชาย:0,หญิง:0,รวม:0}, m4:{ชาย:0,หญิง:0,รวม:0} };
  });
  apps.forEach(function(a){
    const k = String(a.CREATED_AT || '').slice(0,10);
    if (!days30[k]) return;
    const lv = a.LEVEL === 'ม.4' ? 'm4' : 'm1';
    const g  = a.GENDER === 'หญิง' ? 'หญิง' : 'ชาย';
    days30[k][lv][g] = (days30[k][lv][g] || 0) + 1;
    days30[k][lv]['รวม'] = (days30[k][lv]['รวม'] || 0) + 1;
    days30[k].total = (days30[k].total || 0) + 1;
  });
  ...
  const result = {
    ...
    topSchools,
    byProvince,
    byGender: byGenderOf(apps),
    m1ByGender: byGenderOf(m1),
    m4ByGender: byGenderOf(m4),
    m1ByStudyPlan: byPlanOf(m1),
    m4ByStudyPlan: byPlanOf(m4),
...
/* ─────────────────────────────────────────────────────────────
   ...
   เงื่อนไข: สถานะต้องเป็น "รายงานตัวแล้ว"
   ───────────────────────────────────────────────────────────── */
function generateEnrollmentPdf(appId) {
  try {
    ...
    if (String(app.STATUS || '') !== 'รายงานตัวแล้ว') {
      return { ok:false, msg:'สามารถดาวน์โหลด PDF ได้เมื่อสถานะเป็น "รายงานตัวแล้ว" เท่านั้น' };
    }
...
row2('โทรศัพท์นักเรียน', a.PHONE, 'โรงเรียนเดิม', a.OLD_SCHOOL) + row2('ระดับเดิม', a.OLD_LEVEL, 'ผลการเรียน (ปพ.1)', a.GPA) +
...
'</div></div>' +
'</div></div>' +
'<div class="doc-foot"><div>ออกเอกสารจากระบบเมื่อสถานะเป็น “รายงานตัวแล้ว” เท่านั้น</div><div>เลขที่ใบสมัคร ' + _v(a.APP_ID) + '</div></div>' +
'</div></body></html>';
