// v1_22_16: PDPA FIX (วิกฤต/สูง) — ปิดช่องโหว่ 5 จุดที่เรียกตรงผ่าน google.script.run ได้โดยไม่ต้อง login
// v1_22_11: dailyTrend แสดงทุกวันตั้งแต่เปิดรับสมัคร (M1/M4_REG_START) ถึงวันปิดรับสมัคร (M1/M4_REG_END) หรือวันนี้แล้วแต่น้อยกว่า
...
/* =============================================================
   ...
   Student Enrollment System  v1.22.16
   ─────────────────────────────────────────────────────────────
   Sheets   : Applications | Users | Settings | AuditLog
              ExamConfig | ExamScores
   Roles    : ADMIN | STAFF
   Run once : setupSystem()  ←  สร้าง Sheet + default admin (ครั้งแรกไม่ต้องมี token)
   Default  : admin / admin1234
   ─────────────────────────────────────────────────────────────
   CHANGELOG
   v1.22.16 – 2569-07-30 – PDPA FIX (วิกฤต/สูง): ปิดช่องโหว่เรียกตรงผ่าน google.script.run
              1) cancelApplicationByStudent: เพิ่มยืนยัน idCard+birthdate (เดิมไม่มีเลย +
                 APP_ID เดาได้ง่าย → ใครก็ลบใบสมัครใครก็ได้)
              2) generateEnrollmentPdf: เพิ่ม token+VIEW_APPS (เดิมดาวน์โหลด PDF ใครก็ได้)
              3) _supaReq/_supaUpsert/_supaPatch/_supaDelete/_supaGetAll → ห่อเป็น _Supa
                 namespace object (ไม่ใช่ top-level function อีกต่อไป จึงเรียกตรงจาก
                 google.script.run ไม่ได้แล้ว — เดิมใครก็อ่าน/แก้/ลบข้อมูลทั้งตาราง
                 Supabase ได้โดยไม่ต้อง login)
              4) uploadPhoto: เพิ่มยืนยัน idCard+birthdate เมื่อ appId ไม่ใช่ tmp_ (แก้ไข
                 ใบสมัครที่มีอยู่แล้ว) — ตอนสมัครครั้งแรกยังไม่ต้องยืนยัน (ยังไม่มี appId จริง)
              5) setupSystem/fixHeaders/installAutoSyncTrigger/uninstallAutoSyncTrigger:
                 เพิ่ม token+SETTINGS permission (setupSystem ยกเว้นตอน deploy ครั้งแรกสุด
                 ที่ Users sheet ยังว่าง ป้องกัน deadlock bootstrap)
...
const SH = { APP:'Applications', USERS:'Users', SETTINGS:'Settings', LOGS:'AuditLog',
             EXAM_CFG:'ExamConfig', EXAM_SCR:'ExamScores', CONSENT:'ConsentLog' };

// ═══ PDPA Phase 2: Privacy Notice + Consent ═══
// bump เลขนี้เมื่อเนื้อหาประกาศความเป็นส่วนตัวเปลี่ยนแปลงสำคัญ (บังคับให้ผู้ใช้เก่ารับทราบซ้ำ)
// ⚠️ เนื้อหา Privacy Notice (ฝั่ง frontend) ควรให้ผู้บริหาร/ผู้มีความรู้ด้านกฎหมายทบทวนก่อนใช้จริงเสมอ
...
// ─── SUPABASE HELPERS ───
// ═══ PDPA FIX (วิกฤต): ห่อเป็น namespace object แทน top-level function ═══
// เหตุผล: GAS deploy แบบ "Anyone" ทำให้ทุก top-level function เรียกได้ตรงจาก
// browser console โดยไม่ผ่านการตรวจสิทธิ์ใดๆ เลย (google.script.run._Supa.delete(...)
// เดิมใครก็เรียกลบ/อ่าน/แก้ข้อมูลทั้งตาราง Supabase ได้ทันทีโดยไม่ต้อง login)
// การห่อเป็น object property (ไม่ใช่ top-level function) ทำให้ GAS ไม่เปิดเผยผ่าน
// google.script.run อีกต่อไป — ฟังก์ชันเหล่านี้เรียกได้เฉพาะจากโค้ด server ฝั่งเดียวกันเท่านั้น
const _Supa = {
  req(method, table, body, params) {
  ...
  upsert(table, record) {
    return this.req('POST', table, record, {'on_conflict': 'app_id'});
  },
  patch(table, appId, updates) {
    return this.req('PATCH', table, updates, {'app_id': 'eq.'+appId});
  },
  delete(table, appId) {
    return this.req('DELETE', table, null, {'app_id': 'eq.'+appId});
  },
  getAll(table) {
    return this.req('GET', table, null, {'select': '*', 'limit': '5000'});
  }
};
...
function setupSystem(token) {
  // ═══ PDPA FIX (สูง): เดิมไม่มีการตรวจสิทธิ์เลย ใครก็เรียก setupSystem() ตรงๆ
  // ผ่าน google.script.run ได้โดยไม่ต้อง login — เพิ่มการตรวจสิทธิ์ SETTINGS
  // ข้อยกเว้นจำเป็น: การ deploy ครั้งแรกสุด (Users sheet ยังว่าง/ไม่มี) ยังไม่มี
  // admin คนไหนให้ login เลย ถ้าบังคับ token ตั้งแต่ตอนนั้นจะ deploy ระบบไม่ได้เลย
  // (deadlock) จึงอนุญาตให้รันได้โดยไม่ต้องมี token "เฉพาะตอน Users sheet ยังว่าง"
  // เท่านั้น — พอมี user แล้ว (ตั้งแต่ครั้งที่ 2 เป็นต้นไป) จะบังคับ token เสมอ
  const _uShCheck = _sheet(SH.USERS);
  const _isFirstEverSetup = !_uShCheck.getLastRow();
  if (!_isFirstEverSetup) {
    // หมายเหตุสำหรับนักพัฒนา: ถ้าจะรันจาก Apps Script Editor โดยตรง (ไม่ผ่านเว็บ)
    // ให้ login ผ่านหน้าเว็บก่อน 1 ครั้ง แล้วคัดลอก token จาก localStorage
    // ('enroll_token') มาใส่เป็น argument ตอนกด Run ในนี้แทน
  ...
  // ─── ConsentLog (PDPA Phase 2) ───
  const cnSh = _sheet(SH.CONSENT);
  if (!cnSh.getLastRow()) {
    cnSh.appendRow(['CONSENT_ID','ROLE','OWNER_ID','OWNER_NAME','NOTICE_VERSION','CONSENTED_AT']);
    cnSh.getRange('A1:F1').setFontWeight('bold').setBackground('#ede9fe');
  }
...
// ดึงเลขเวอร์ชันประกาศความเป็นส่วนตัวปัจจุบัน (public — ใช้ได้โดยไม่ต้อง login)
function getPrivacyNoticeVersion() {
  return { ok:true, version: PRIVACY_NOTICE_VERSION };
}
...
// ═══ PDPA Phase 2: Consent (staff/admin) ═══
// ทั้งสองฟังก์ชันนี้ดึงตัวตนจาก session เท่านั้น (sessionArgs pattern) — ไม่รับ
// ownerId/ownerName จาก client เลย ป้องกันคนหนึ่งบันทึก/ปลอมความยินยอมแทนอีกคน
function getMyConsentStatus(token) {
  const sess = _requireAuth(token);
  const rows = _sheetData(SH.CONSENT);
  const mine = rows.filter(function(r) {
    return String(r.OWNER_ID) === String(sess.username) &&
           String(r.NOTICE_VERSION) === PRIVACY_NOTICE_VERSION;
  });
  return { ok:true, consented: mine.length > 0, version: PRIVACY_NOTICE_VERSION };
}

function recordMyConsent(token) {
  const sess = _requireAuth(token);
  return _withLock(function() {
    const sh = _sheet(SH.CONSENT);
    const consentId = Utilities.getUuid();
    sh.appendRow([consentId, sess.role, sess.username, sess.name, PRIVACY_NOTICE_VERSION, new Date()]);
    _log(sess.name, 'CONSENT_RECORDED', 'version:' + PRIVACY_NOTICE_VERSION);
    return { ok:true };
  });
}
...
function submitApplication(data) {
  ...
  // ═══ PDPA Phase 2: บังคับรับทราบประกาศความเป็นส่วนตัวก่อนส่งใบสมัครเสมอ ═══
  // หมายเหตุ: การกด "รับทราบ" นี้เป็นการบันทึกว่าผู้กรอกเห็น/รับทราบประกาศเท่านั้น
  // ไม่ใช่ความยินยอมของผู้ปกครองตามกฎหมายที่โรงเรียนต้องจัดการแยกต่างหากนอกระบบ
  if (!data.PRIVACY_CONSENT)
    return { ok:false, msg:'กรุณารับทราบประกาศความเป็นส่วนตัวก่อนส่งใบสมัคร' };
  ...
  try { _Supa.upsert('applications', _toSupaRec(Object.assign({}, data, { APP_ID:appId, STATUS:'สมัครแล้ว', CREATED_AT:nowTs, UPDATED_AT:nowTs }))); }
  catch(e) { Logger.log('Supabase write failed: '+e); }

  // ── PDPA Phase 2: บันทึกว่าผู้สมัครรับทราบประกาศความเป็นส่วนตัวแล้ว ──
  try {
    _sheet(SH.CONSENT).appendRow([
      Utilities.getUuid(), 'APPLICANT', String(data.ID_CARD).trim(),
      (data.FNAME||'')+' '+(data.LNAME||''), PRIVACY_NOTICE_VERSION, nowTs
    ]);
  } catch(e) { Logger.log('Consent log failed: '+e); }
...
function cancelApplicationByStudent(idCard, birthdate, appId, reason) {
  // ═══ PDPA FIX (วิกฤต): เดิมไม่มีการยืนยันตัวตนเลย + APP_ID เป็นเลขนับเรียง
  // (68-M1-0001, 68-M1-0002, ...) เดา/ไล่เลขได้ง่ายมาก ทำให้ใครก็เรียกลบใบสมัคร
  // ของนักเรียนคนไหนก็ได้ทั้งระบบผ่าน google.script.run โดยตรง ไม่ต้อง login
  // แก้ให้ตรวจ idCard+birthdate แบบเดียวกับ updateApplicationByStudent() เสมอ ─
  ...
  return _withLock(() => {
    ...
    try { _Supa.delete('applications', appId); } catch(e) {}
  ...
  return _withLock(function() {
    ...
    if (ok) {
      ...
      try {
        ...
        _Supa.patch('applications', appId, supaUpdates);
...
function getPublicStats() {
  ...
  // เพิ่ม daily trend (ตั้งแต่วันเปิดรับสมัคร → วันปิดรับสมัคร หรือวันนี้ แล้วแต่น้อยกว่า)
  (function() {
    var startRaw = cfg.M1_REG_START || cfg.M4_REG_START || '';
    var endRaw   = cfg.M1_REG_END   || cfg.M4_REG_END   || '';
    var today = new Date(); today.setHours(0,0,0,0);

    var startD;
    if (startRaw) {
      startD = new Date(startRaw); startD.setHours(0,0,0,0);
    } else {
      startD = new Date(today); startD.setDate(startD.getDate() - 9);
    }

    // ปลาย = วันปิดรับสมัคร (ถ้ามี) แต่ไม่เกินวันนี้
    var endD;
    if (endRaw) {
      endD = new Date(endRaw); endD.setHours(0,0,0,0);
  ...
  if (updates.length) {
    return _withLock(function() {
      updates.forEach(function(u) {
        ...
        // Supabase sync
        try { _Supa.patch('applications', u.appId, { status:u.newStatus, score:String(u.totalScore), updated_at:new Date().toISOString() }); } catch(e){}
      });
...
function addAdminUser(token, username, password, name, role) {
  ...
  // ═══ PDPA Phase 3 (XSS defense-in-depth): จำกัดอักขระของ username ตั้งแต่ต้นทาง
  // ป้องกัน HTML/JS special character (< > " ' &) หลุดเข้าไปได้ตั้งแต่แรก ═══
  if (!/^[A-Za-z0-9_.-]{3,50}$/.test(String(username).trim()))
    return { ok:false, msg:'ชื่อผู้ใช้ต้องเป็นตัวอักษรอังกฤษ/ตัวเลข/_-. เท่านั้น (3-50 ตัวอักษร)' };
...
// ─── UPLOAD PHOTO ───
function uploadPhoto(base64Data, appId, idCard, birthdate) {
  // ═══ PDPA FIX (สูง): เดิมไม่มีการตรวจความเป็นเจ้าของเลย — ใครก็เรียกอัปโหลด/
  // เขียนทับรูปผูกกับ appId ของใบสมัครคนไหนก็ได้ (APP_ID เดาได้ง่าย เป็นเลขนับเรียง)
  // ยกเว้นตอนสมัครครั้งแรก (appId ยังเป็น 'tmp_...' เพราะยังไม่มีใบสมัครจริงให้ตรวจสอบ
  // ความเป็นเจ้าของได้) — กรณีนั้นคงเปิดไว้ตามเดิม แต่ถ้าเป็น appId จริงแล้ว (ตอนแก้ไข)
  // ต้องยืนยัน idCard+birthdate แบบเดียวกับ updateApplicationByStudent() ก่อนเสมอ
  const isTmp = String(appId||'').indexOf('tmp_') === 0;
  if (!isTmp) {
    const verify = verifyOwnerAndGetApp(idCard, birthdate);
    if (!verify.ok) return verify;
    if (String(verify.app.APP_ID) !== String(appId))
...
function syncAllToSupabase(token) {
  ...
  try {
    ...
    apps.forEach(function(a) {
      const r = _Supa.upsert('applications', _toSupaRec(a));
...
function syncSheetToSupabase(token) {
  ...
  let deleted = 0;
  try {
    const supaRows = _Supa.getAll('applications');
...
/**
 * installAutoSyncTrigger(token, intervalHours)
 * ─────────────────────────────────────────────────────
 ...
 *     PDPA FIX: ต้องส่ง token ของ admin ที่ login แล้ว (ดูวิธีหา token ที่คอมเมนต์
 *     ใน setupSystem() ด้านบน)
 *
 * วิธีใช้:
 ...
 */
function installAutoSyncTrigger(token, intervalHours) {
  // ═══ PDPA FIX (สูง): เดิมไม่มีการตรวจสิทธิ์เลย ใครก็ติดตั้ง/แก้ trigger sync ได้ ══
  _requirePerm(token, 'SETTINGS');
...
/**
 * uninstallAutoSyncTrigger(token)
 * ─────────────────────────────────────────────────────
 ...
 */
function uninstallAutoSyncTrigger(token) {
  // ═══ PDPA FIX (สูง): เดิมไม่มีการตรวจสิทธิ์เลย ใครก็ลบ trigger sync ได้ ══
  _requirePerm(token, 'SETTINGS');
  var count = 0;
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (t.getHandlerFunction() === '_autoSyncFromTrigger') {
...
/* ═══════════════════════════════════════════════════════════════════
   PDPA Phase 3 — Data Retention
   ───────────────────────────────────────────────────────────────────
   หลักสำคัญ: แยกให้ชัดว่าอะไรคือ "หลักฐานทางวิชาการ/ความยินยอม" (ห้ามลบอัตโนมัติ)
   กับอะไรคือ "operational/security log" (ลบอัตโนมัติได้)

   ✅ ลบอัตโนมัติได้ (ใช้ pattern นี้)  : AuditLog (SH.LOGS) — เก็บ 1 ปี
      Session token ก็ลบอัตโนมัติอยู่แล้วโดยธรรมชาติ (CacheService TTL 3 ชม.
      ใน _createSession/login ไม่ต้องทำ purge เพิ่ม)
   ❌ ห้ามลบอัตโนมัติเด็ดขาด : Applications, ExamScores, ConsentLog
      (เป็นหลักฐานทางวิชาการ/ความยินยอมตามกฎหมาย — ถ้าจะ "ลบตามคำขอ" ต้องมี
      กระบวนการยืนยันตัวตน+อนุมัติแยกต่างหาก ไม่ใช่ trigger อัตโนมัติ)
   ═══════════════════════════════════════════════════════════════════ */

/**
 * installAuditLogPurgeTrigger(token)
 * ─────────────────────────────────────────────────────
 * ⚠️  รันใน GAS Editor โดยตรงเท่านั้น (ครั้งเดียวตอน setup)
 * ติดตั้ง trigger รายวัน (ตี 3 — กระทบผู้ใช้น้อยสุด) ให้ purgeOldAuditLogs()
 * ลบ AuditLog ที่เก่ากว่า 1 ปีทิ้งอัตโนมัติ
 * PDPA FIX: ต้องส่ง token ของ admin ที่ login แล้ว (ดูวิธีหา token ที่คอมเมนต์
 * ใน setupSystem() ด้านบน)
 */
function installAuditLogPurgeTrigger(token) {
  _requirePerm(token, 'SETTINGS');
  // ลบ trigger เดิมก่อนกันติดตั้งซ้ำซ้อน
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (t.getHandlerFunction() === 'purgeOldAuditLogs') ScriptApp.deleteTrigger(t);
  });
  ScriptApp.newTrigger('purgeOldAuditLogs').timeBased().everyDays(1).atHour(3).create();
  _log('SYSTEM', 'INSTALL_AUDIT_PURGE_TRIGGER', 'เก็บ AuditLog 1 ปี ลบอัตโนมัติทุกวันตอนตี 3');
  Logger.log('✅ installAuditLogPurgeTrigger: ติดตั้งสำเร็จ');
  return { ok:true, msg:'ติดตั้ง trigger สำเร็จ — purgeOldAuditLogs() จะรันทุกวันตอนตี 3' };
}

/**
 * uninstallAuditLogPurgeTrigger(token)
 * ─────────────────────────────────────────────────────
 * ⚠️  รันใน GAS Editor โดยตรงเท่านั้น
 * PDPA FIX: ต้องส่ง token ของ admin ที่ login แล้ว
 */
function uninstallAuditLogPurgeTrigger(token) {
  _requirePerm(token, 'SETTINGS');
  var count = 0;
  ScriptApp.getProjectTriggers().forEach(function(t) {
    if (t.getHandlerFunction() === 'purgeOldAuditLogs') { ScriptApp.deleteTrigger(t); count++; }
  });
  _log('SYSTEM', 'UNINSTALL_AUDIT_PURGE_TRIGGER', count + ' trigger(s) ถูกลบ');
  Logger.log('✅ uninstallAuditLogPurgeTrigger: ลบ ' + count + ' trigger(s) สำเร็จ');
 ...
 * purgeOldAuditLogs()
 * ─────────────────────────────────────────────────────
 * Entry point สำหรับ GAS Time-based Trigger เรียก (ไม่ต้องการ token — เหมือน
 * _autoSyncFromTrigger) — ลบแถวใน AuditLog ที่เก่ากว่า 365 วันทิ้งอัตโนมัติ
 * เก็บแถว header (แถวแรก) ไว้เสมอไม่ว่าจะมีข้อมูลเหลือหรือไม่
 */
function purgeOldAuditLogs() {
  ...
  try {
    return _withLock(function() {
      ...
      if (lastRow < 2) return { ok:true, removed:0 }; // แค่ header หรือว่างเปล่า
      ...
      data.slice(1).forEach(function(row) {
        ...
        // แถวที่ parse วันที่ไม่ได้ → เก็บไว้ก่อนเพื่อความปลอดภัย (ไม่เสี่ยงลบผิด)
        if (isNaN(d.getTime()) || d >= cutoff) keepRows.push(row);
        else removed++;
      });
      ...
      Logger.log('purgeOldAuditLogs: ลบ ' + removed + ' แถวที่เก่ากว่า ' + RETENTION_DAYS + ' วัน');
      // บันทึกว่ามีการ purge เกิดขึ้น (แถวนี้เองจะถูกลบในรอบถัดไปเมื่อครบ 1 ปีเช่นกัน)
      if (removed > 0) _log('SYSTEM', 'PURGE_AUDIT_LOG', removed + ' แถว (เก็บ ' + RETENTION_DAYS + ' วัน)');
      return { ok:true, removed: removed };
    });
  } catch(e) {
    Logger.log('purgeOldAuditLogs error: ' + e);
    return { ok:false, msg: String(e) };
  }
}
    ...
    try {
      ...
      if (supaRows && Array.isArray(supaRows)) {
        ...
        supaRows
          ...
          .filter(function(id){ return id && !sheetIds.has(id); })
          .forEach(function(id) {
            try { _Supa.delete('applications', id); deleted++; } catch(e) {}
          });
      }
    } catch(e) {}
...
function fixHeaders(token) {
  // ═══ PDPA FIX (สูง): เดิมไม่มีการตรวจสิทธิ์เลย (ทำงานเหมือน migrateSheet()
  // ที่ป้องกันไว้แล้ว แต่ฟังก์ชันนี้หลุดรอด) ══
  _requirePerm(token, 'SETTINGS');
  _migrateAppSheet();
}
...
/* ─────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────── */
function generateEnrollmentPdf(token, appId) {
  // ═══ PDPA FIX (วิกฤต): เดิมไม่มีการตรวจสิทธิ์เลย รับแค่ appId (เดา/ไล่เลขได้ง่าย)
  // → ใครก็ดาวน์โหลด PDF ใบสมัครฉบับเต็ม (ชื่อ ที่อยู่ เลขบัตร ปชช. ข้อมูลผู้ปกครอง
  // สุขภาพ) ของนักเรียนคนไหนก็ได้ โดยไม่ต้อง login — เพิ่ม token + ตรวจสิทธิ์ VIEW_APPS
  // (ฟังก์ชันนี้เป็นเวอร์ชันเดิมที่ frontend ปัจจุบันไม่ได้เรียกใช้แล้ว — ใช้
  // generateAndSaveEnrollmentPdf(token,...) แทน ซึ่งป้องกันไว้อยู่แล้ว — คงไว้เผื่อใช้
  // ในอนาคตแต่ต้องปิดช่องโหว่นี้ก่อน)
  try {
    _requirePerm(token, 'VIEW_APPS');
  } catch(e) {
    return { ok:false, msg: e && e.message ? e.message : 'ไม่มีสิทธิ์ดำเนินการนี้' };
  }
  try {
    appId = String(appId || '').trim();
