...
/* =============================================================
   ...
   Student Enrollment System  v1.22.12
   ─────────────────────────────────────────────────────────────
...
function _savePdfToDrive(app, cfg) {
  try {
    ...
    // ลบไฟล์เก่าถ้ามี (ป้องกัน duplicate)
    ...
    return { ok:true, url:url, filename:blob.getName() };
  } catch(e) {
...
'html,body{background:#fff;font-family:"Sarabun",sans-serif;color:#111;-webkit-print-color-adjust:exact;print-color-adjust:exact;font-size:8.4pt;line-height:1.28}'+
'.sheet{width:100%;background:#fff;padding:0;position:relative;display:flex;flex-direction:column;min-height:calc(297mm - 16mm)}'+
