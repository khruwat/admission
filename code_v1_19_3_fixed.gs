...
const SETTINGS_DEFAULTS = [
  ...
  ['CHANGELOG_TEXT',     'v1.19.0 – 2569-03-17 – ระบบคะแนนสอบ + Export Google Sheet\nv1.18.0 – 2569-03-17 – Export รายชื่อ PDF + Excel แบบมืออาชีพ\nv1.17.0 – 2569-03-17 – รอบ 2 + ย้ายเข้าไม่มีวันปิด + ตารางรายวันตามช่วงจริง'],
  ['ANNOUNCEMENT',       ''],
  ['ANNOUNCE_IMAGE',     ''],
  ['ANNOUNCE_POPUP',     'true'],
...
function uploadLogo(token, base64Data) {
  ...
  try {
    ...
    // หา folder — ใช้ root Drive ถ้า _getPhotoFolder() ยังไม่มี
    let folder;
    try {
      folder = _getPhotoFolder();
    } catch(fe) {
      Logger.log('_getPhotoFolder failed: ' + fe);
      // fallback: เก็บที่ root Drive
      folder = DriveApp.getRootFolder();
    }

    // ลบโลโก้เดิมทุก extension ที่อาจมีอยู่
    ...
    // ใช้ thumbnail URL ที่เปิดได้ใน <img> tag โดยตรง
    const fileId = file.getId();
    const url    = 'https://drive.google.com/thumbnail?id=' + fileId + '&sz=w200';

    // บันทึก LOGO_URL กลับไป settings
