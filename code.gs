...
function submitApplication(data) {
  ...
  return _withLock(() => {
    ...
    const last = sh.getLastRow();
    if (!last) sh.appendRow(APP_COLS);

    const vals  = last >= 2 ? sh.getRange(1,1,last,APP_COLS.length).getValues() : [APP_COLS.map(c=>c)];
    const hdrs  = vals[0].map(h => String(h).trim());
    ...
    for (let r = 1; r < vals.length; r++) {
      if (String(vals[r][iId]).trim() === String(data.ID_CARD).trim()) {
        // เลขบัตรซ้ำ — ห้ามสมัครซ้ำไม่ว่าระดับชั้นใด
        return { ok:false, msg:'เลขประจำตัวประชาชนนี้ได้สมัครแล้ว (เลขที่ใบสมัคร: '+vals[r][iApId]+') หากต้องการแก้ไขกรุณาติดต่อเจ้าหน้าที่' };
      }
    }

    const yr    = String(cfg.ACADEMIC_YEAR||'2568').slice(-2);
    const pfx   = data.LEVEL === 'ม.1' ? 'M1' : 'M4';
    const cnt   = vals.slice(1).filter(r => r[iLv] === data.LEVEL).length + 1;
    const appId = yr+'-'+pfx+'-'+String(cnt).padStart(4,'0');
    const nowTs = new Date();

    // TEXT_COLS ทุกฟิลด์บังคับ String() เพื่อป้องกัน Sheets ตัด 0 นำหน้า
    ...
    sh.appendRow(APP_COLS.map(c => rec[c] !== undefined ? rec[c] : ''));
...
// เพิ่ม column ใหม่ที่ยังไม่มีใน sheet (ไม่ลบ column เดิม)
function _migrateAppSheet() {
  const sh   = _sheet(SH.APP);
  const last = sh.getLastRow();
  if (!last) { sh.appendRow(APP_COLS); return; }
  ...
  APP_COLS.forEach(col => {
    if (!hdrs.includes(col)) {
      ...
      sh.getRange(1, newIdx).setValue(col).setFontWeight('bold').setBackground('#e0f2fe').setNumberFormat('@');
      hdrs.push(col);
    }
  });
}
